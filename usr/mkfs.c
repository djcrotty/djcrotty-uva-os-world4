...
// fxl: from xv6. very simple. only make files, no dirs
...
main(int argc, char *argv[])
{
  ...
  nmeta = 2 + nlog + ninodeblocks + nbitmap;  // fxl:# of "meta" blocks
  ...
  // fxl: create all files passed in
  for(i = 2; i < argc; i++){  
    // get rid of "user/"
    char *shortname;
...
void    // fxl: read from a sector
rsect(uint sec, void *buf)        
{
  if(lseek(fsfd, sec * BSIZE, 0) != sec * BSIZE)
    die("lseek");
  if(read(fsfd, buf, BSIZE) != BSIZE)
    die("read");  // fxl: when this failed, why (reports "Success"
}

uint    // fxl: alloc inode and zero fill
ialloc(ushort type)
{
  uint inum = freeinode++;
  struct dinode din;
...
// the function marks [0,used) blocks as "in use" in bitmap. 
...
// append data (xp, bytes: n) to inode
// quest: large files 
void        
iappend(uint inum, void *xp, int n)
{
  ...
  while(n > 0){   // n: # remaining bytes to write
    ...
    assert(fbn < MAXFILE);  // used to be ~270KB filesize limit...
    if(fbn < NDIRECT){ // direct ptr
      if(xint(din.addrs[fbn]) == 0){
        din.addrs[fbn] = xint(freeblock++); // alloc & link free blk
      }
      x = xint(din.addrs[fbn]);  // x: blk addr
    } else if (fbn < NDIRECT + NINDIRECT) {  // indirect ptr
