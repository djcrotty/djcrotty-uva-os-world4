...
// fxl: meaning the app has "returned" the brk memory already 
...
// fxl: meaing if inode was locked?
void
iputtest(char *s)
{
  if(mkdir("iputdir") < 0){
...
pipe1(char *s)
{
  ...
  if(pid == 0){
    ...
    close(fds[1]);
    total = 0;
    cc = 1; // fxl: give it expoentially larger buf to read....
...
forkforkfork(char *s)
{
  ...
  if(pid == 0){  // child
    while(1){
      ...
      pid = fork(); 
      if(pid < 0 || pid > 40){  // fxl: Mar 2024: this works
      // if(pid < 0 ){    // fxl: this failed. maybe b/c we cannot recycle pids. to be fixed
...
// fxl: can't expect to work with fat32 which only has 1 link
void
unlinkread(char *s)
{
  enum { SZ = 5 };
...
// fxl: fat can finish first few, until link() which it does not support
void
subdir(char *s)
{
  int fd, cc;
...
{
  // enum { N = 20, SZ=600 };
  // NB: N cannot be >255, cf memset() below
  enum { N = 200, SZ=8*1024}; // fxl: test enlarged files (doubly indirect ptr in inode)
  int fd, i, total, cc;
  ...
  total = 0;
  for(i = 0; ; i++){    // fxl: each read SZ/2 bytes
    cc = read(fd, buf, SZ/2);
    if(cc < 0){
...
// fxl: we passed this but failed fork() in subsequent countfree(), b/c we
//    dont recycle pids.
void
forktest(char *s)
{
...
Functions for Memory Model Aware Atomic Operations" by fxl */
int counter = 100; 
...
sbrkbasic(char *s)
{
  ...
  for(i = 0; i < 5000; i++){
    b = sbrk(1);
    if(b != a){     // fxl: a - old brk
      printf("%s: sbrk test failed %d %x %x\n", s, i, a, b);
      exit(1);
    }
    *b = 1;     // fxl: write to new brk
    a = b + 1;      
  }
  pid = fork();
  if(pid < 0){
...
// fxl:  requires efficient rep of task va (otherwise cannot grow to ~100MB)
void
sbrkmuch(char *s)
{
...
// fxl: fork a bunch of tasks to sweep kernel va ...
void
kernmem(char *s)
{
  char *a;
  ...
  for(a = (char*)(KERNBASE); a < (char*) (KERNBASE+2000000); a += 50000){
    ...
    if(pid == 0){
      // fxl: if ever printed out, then task can access kern va. 
...
// #include "../kernel/plat-virt.h"
#define MAXVA USER_VA_END
...
sbrkfail(char *s)
{
  ...
  // fxl: spawn a bunch of tasks, each allocating a lot ...
...
stacktest(char *s)
{
#if 0  // fxl: TBD as we have on demand stack growth
  int pid;
  int xstatus;
...
countfree()
{
  ...
  if(pid == 0){
    close(fds[0]);
    printf("countfree starts. brk is 0x%x\n", sbrk(0));
    // fxl: use a loop to sbrk() & count pages. for each page write a char to the pipe
    while(1){
      uint64 a = (uint64) sbrk(4096);
      ...
      *(char *)(a + 4096 - 1) = 1;
      ...
      // report back one more page.   fxl: write to pipe
  ...
  // fxl: use a loop to receive chars from the pipe...
  close(fds[1]);
...
drivetests(int quick, int continuous, char *justone) {
  do {
    ...
    if((free1 = countfree()) < free0) { // fxl: will count free again...
