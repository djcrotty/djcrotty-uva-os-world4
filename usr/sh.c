/* Shell from xv6
 * Quite simple. Runs an input loop, reads input line, forks,
 * parses into several major cmd types (exec, redir, pipe...)
 * and executes.
 */
#include "user.h"
...
// generic cmd type, will be cast into the following XXXcmd structs
struct cmd {
  int type;
};
...
/* Create key procfs and devfs files 
   return 0 if ok
*/
// quest: complete sh
