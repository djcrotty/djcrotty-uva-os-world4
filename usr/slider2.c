 ...
 */
 ...
 void* BMP_Load(const char *filename, int *width, int *height) {
   ...
   for (int i = 0; i < h; i ++) {
     ...
     nread = read(fd, p, 3*w);  assert(nread==3*w); // read a row
     ...
     for (int j = 0; j < w; j++) {
       ...
       uint8_t g = p[j * 3 + 1]; // g
       uint8_t r = p[j * 3 + 2]; // b
       pixels[w * i + j] = (r << 16) | (g << 8) | b | 0xFF000000; // ARGB
     }
   }
 ...
 int render() {
   ...
   if (config_isfb) {  
     ...
     /* STUDENT_TODO: your code here */
     set_bkgnd(0x00ffffff /*white*/, pitch, dispinfo[VHEIGHT]); // clear fb
     for (y = 0; y < fb_h; y++) {
       lseek(fb, y * pitch, SEEK_SET);
       if (write(fb, (unsigned char*)pixels + y * w * PIXELSIZE, fb_w * PIXELSIZE) < fb_w * PIXELSIZE) {
 ...
 int main(int argc, char **argv) {
   ...
   while (1) {
     ...
     if (n || scancode == 0 || scancode >= NUM_SCANCODES || evtype == INVALID) {
