...
void DWHCIFrameSchedulerNoSplitWaitForFrame (TDWHCIFrameScheduler *pBase)
{
	...
	unsigned sec, msec, sec2, msec2;
	current_time(&sec, &msec); 

	TDWHCIRegister FrameNumber;
	DWHCIRegister (&FrameNumber, DWHCI_HOST_FRM_NUM);

	// (xzl( Attempt to wait for the next frame.
	// On QEMU, the CPU runs much faster than the I/O operations, 
	// so the CPU might not read continuous frame numbers.
	// Additionally, since DWHCI_MAX_FRAME_NUMBER is 0x3FFF, 
	// there is a possibility of frame number wrap-around.
