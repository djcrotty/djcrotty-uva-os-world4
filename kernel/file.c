...
void fileclose(struct file *f) {
    ...
    */
    ...
    if (ff.type == FD_PROCFS && ff.content) {
        ...
        if (st->usize)
            procfs_parse_usercontent(&ff);  // project idea: complete this (where to place this call??)
        kfree(ff.content); 
...
int fileread(struct file *f, uint64 addr, int n) {
    ...
    if (f->type == FD_PIPE) {
        ...
        if (f->major < 0 || f->major >= NDEV || !devsw[f->major].read)
            return -1;
        r = devsw[f->major].read(1, addr, 0/*off*/, n, f->nonblocking?0:1, f->content); // device read
...
// project idea: make students implement this for seek in /dev/fb (nes etc) (also game file in doom)
#include "fb.h"
...
int filelseek(struct file *f, int offset, int whence) {
...
#ifdef CONFIG_FAT    // FAT32 specific
    else if (f->type == FD_INODE_FAT) { 
    ...
    case SEEK_SET:
        ...
        break;
    case SEEK_CUR:
        newoff = f->off + offset; /* Calculate new offset based on current offset and provided offset */ /* STUDENT_TODO: replace this */
        break; 
    case SEEK_END:  // "set file offset to EOF plus offset", i.e. offset shall <0
        if (offset > 0) { W("unsupported"); return -1;}
        newoff = size + offset; /* Calculate new offset based on file size and provided offset */ /* STUDENT_TODO: replace this */
        break; 
    default:
        E("unrecog option");
        return -1; 
    ...
    if (newoff > size) /* STUDENT_TODO: replace this */
...
int devfb_write(int user_src, uint64 src, int off, int n, void *content) {
    ...
    if (either_copyin(the_fb.fb + off, user_src, src, len) == -1) /* STUDENT_TODO: replace this */
        goto out; 
    ret = len;
    __asm_flush_dcache_range(the_fb.fb + off, the_fb.fb + off + len); /* Flush the framebuffer range */
...
int procfs_gen_content(int major, char *txtbuf, int sz) {
    ...
    case PROCFS_DISPINFO:
        acquire(&mboxlock);
        // generate the content for user to read from /proc/dispinfo
        ...
        break;    
    case PROCFS_FBCTL: 
        // generate the content for user to read  from /proc/fbctl
        acquire(&mboxlock);
        len = snprintf(txtbuf, sz, 
    ...
    case PROCFS_SBCTL: 
        ...
        break; 
#endif        
    case PROCFS_CPUINFO: //cpu util, e.g. "10 20 30 40"     project idea: impl this
        {
...
//  project idea: should we maintain read offset? if so, how? 
static int procfs_read(struct file *f, uint64 dst, uint n) {    
    uint len, off, sz; 
    char *buf; 
...
int procfs_parse_usercontent(struct file *f) {
    ...
    switch (f->major)
    {
    case PROCFS_FBCTL:      // project idea: complete this 
