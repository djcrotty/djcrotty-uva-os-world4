...
/*
	...
	sys_sleep() is no longer based on sched ticks; it is based on ktimer (ARM
	system timers), thus more fine-grained. However, the timing may still be
	affected by sched ticks (generic timer) because when a task calls sleep(), the
	CPU switches to the idle task, which only calls schedule() at the next sched
	tick. Thus, any RUNNABLE task may not run immediately.
	
	This might be a problem for NES, which has a timer task that calls the sleep()
	syscall periodically.
	
	Possible solutions:
	1. Change the application. For example, NES could get rid of the timer task
	   and use non-blocking I/O for keyboard events. This depends on
	   non-blocking I/O.
	2. Change the scheduler behavior, which requires deeper changes.
	
	As of 10/23/24, leave as is. NES on WSL can get ~37 FPS with SCHED_TICK_HZ=10.
*/
 ...
 * 
 *  when emulated on qemu, arm generic timer is at 1MHz by default
 */
