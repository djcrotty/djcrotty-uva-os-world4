...
int move_to_user_mode(unsigned long start, unsigned long size, unsigned long pc) {
    ...
    // project idea: map the fb area via map_page() here, so that the 1st user process
    // can write to fb... and can draw donuts 
    // project idea: subsequent fork() needs to inhert that mapping, so that 
